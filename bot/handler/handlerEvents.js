...
async function onStart() {
  ...
  if (!hideNotiMessage.commandNotFound) {
    const commandList = Array.from(GoatBot.commands.keys());
    const suggestions = commandList.filter(cmd => cmd.startsWith(commandName));
    
    let replyMessage;
    if (commandName) {
      if (suggestions.length > 0) {
        replyMessage = `Hey Bby~ 🤔 Did you mean '${commandName}'? Maybe you wanted: ${suggestions.join(', ')}?`;
      } else {
        replyMessage = utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFound", commandName, prefix);
      }
    } else {
      replyMessage = utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFound2", prefix);
    }
